...
const QuickBookingMini = ({ triggerLabel = 'ĐẶT VÉ NHANH', isOpen: controlledOpen, onClose: onCloseProp, onOpen: onOpenProp, showTrigger = true }) => {
  const navigate = useNavigate();
  const [isExpanded, setIsExpanded] = useState(false);
  const isControlled = controlledOpen !== undefined;
  const isOpenState = isControlled ? controlledOpen : isExpanded;
  const setOpen = (open) => {
    if (isControlled && onCloseProp && !open) onCloseProp();
    if (!isControlled) setIsExpanded(open);
  };
  const handleOpen = () => {
    if (isControlled && onOpenProp) onOpenProp();
    else if (!isControlled) setIsExpanded(true);
  };
  const handleClose = () => {
  ...
  return (
    <>
      {/* Trigger Button - ẩn khi mở từ trang khác (vd: Bắp nước) */}
      {showTrigger && !isOpenState && (
        <button className="qbm-trigger" onClick={handleOpen}>
          {triggerLabel}
        </button>
      )}
      ...
      {/* Modal */}
      {isOpenState && (
        <div className="qbm-overlay" onClick={handleClose}>
          <div className="qbm-modal" onClick={(e) => e.stopPropagation()}>
            <div className="qbm-header">
              <h2>ĐẶT VÉ NHANH</h2>
              <button className="qbm-close" onClick={handleClose}>×</button>
            </div>
