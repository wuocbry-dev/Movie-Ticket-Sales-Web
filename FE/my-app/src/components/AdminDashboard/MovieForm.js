...
function formStateFromMovie(movie) {
  const genreIds = Array.isArray(movie.genres) ? movie.genres.map((g) => g.id) : [];
  const formats = Array.isArray(movie.availableFormats) ? movie.availableFormats : [];
  return {
    title: movie.title || '',
...
const MovieForm = ({ movie, genres, onSubmit, onClose }) => {
  ...
  useEffect(() => {
    if (movie) {
      setFormData(formStateFromMovie(movie));
    } else {
  ...
  return (
    <div className="mvf-modal-overlay" role="presentation">
      <div
        className="mvf-modal-content"
        onClick={(e) => e.stopPropagation()}
        role="dialog"
        aria-modal="true"
        aria-labelledby="mvf-form-title"
      >
        <div className="mvf-modal-header">
          <h2 id="mvf-form-title">{movie ? 'Chỉnh sửa phim' : 'Thêm phim mới'}</h2>
          <button type="button" className="mvf-btn-close" onClick={onClose} aria-label="Đóng">
            <FaTimes />
          </button>
        </div>

        <form onSubmit={handleSubmit} className="mvf-movie-form">
          {/* Tab Navigation */}
          <div className="mvf-tab-navigation">
            <button
              type="button"
              className={`mvf-tab-button ${activeTab === 'basic' ? 'active' : ''}`}
            ...
            <button
              type="button"
              className={`mvf-tab-button ${activeTab === 'imdb' ? 'active' : ''}`}
            ...
            <button
              type="button"
              className={`mvf-tab-button ${activeTab === 'media' ? 'active' : ''}`}
          ...
          {activeTab === 'basic' && (
            <div className="mvf-tab-content">
              <div className="mvf-form-grid">
            <div className="mvf-form-section">
              <h3>Thông tin phim</h3>
              
              <div className="mvf-form-group">
                ...
                />
                {errors.title && <span className="mvf-error-message">{errors.title}</span>}
              </div>
              ...
              <div className="mvf-form-group">
                ...
                />
                {errors.titleEn && <span className="mvf-error-message">{errors.titleEn}</span>}
              </div>

              <div className="mvf-form-row">
                <div className="mvf-form-group">
                  ...
                  />
                  {errors.duration && <span className="mvf-error-message">{errors.duration}</span>}
                </div>
              ...
              <div className="mvf-form-row">
                <div className="mvf-form-group">
                  ...
                  />
                  {errors.releaseDate && <span className="mvf-error-message">{errors.releaseDate}</span>}
                </div>
              ...
              <div className="mvf-form-group mvf-checkbox-group">
                <label>
                  <input
                    type="checkbox"
                    name="isFeatured"
            ...
            <div className="mvf-form-section">
              ...
              <div className="mvf-form-group">
                ...
                <div className="mvf-checkbox-grid">
                  {genres.map(genre => (
                    <label key={genre.id} className="mvf-checkbox-label">
                      <input
                        type="checkbox"
                ...
                {errors.genres && <span className="mvf-error-message">{errors.genres}</span>}
              </div>
              ...
              <div className="mvf-form-group">
                ...
                <div className="mvf-checkbox-grid">
                  {['2D', '3D', 'IMAX', '4DX', 'ScreenX'].map(format => (
                    <label key={format} className="mvf-checkbox-label">
                      <input
                        type="checkbox"
            ...
            <div className="mvf-form-section">
              ...
              <div className="mvf-form-group">
                <label>Poster *</label>
                <div className="mvf-upload-container">
                  <div className="mvf-upload-buttons">
                    <label className="mvf-btn-upload" style={{ opacity: uploadingPoster ? 0.6 : 1 }}>
                    ...
                    <span className="mvf-upload-hint">hoặc nhập URL</span>
                  </div>
                  <input
                    type="url"
                    name="posterUrl"
                  ...
                  />
                  {formData.posterUrl && (
                    <div className="mvf-image-preview">
                      <FaImage />
                      <img src={formData.posterUrl} alt="Poster preview" onError={(e) => e.target.style.display = 'none'} />
                    </div>
                  )}
                </div>
                {errors.posterUrl && <span className="mvf-error-message">{errors.posterUrl}</span>}
              </div>
              ...
              <div className="mvf-form-group">
                ...
                <div className="mvf-upload-container">
                  <div className="mvf-upload-buttons">
                    <label className="mvf-btn-upload" style={{ opacity: uploadingBackdrop ? 0.6 : 1 }}>
            ...
            <div className="mvf-form-section">
              ...
              <div className="mvf-form-group">
                ...
                />
                {errors.synopsis && <span className="mvf-error-message">{errors.synopsis}</span>}
              </div>
          ...
          <div className="mvf-form-actions">
            <button type="button" onClick={onClose} className="mvf-btn-cancel">
              Hủy
            </button>
            <button type="submit" className="mvf-btn-submit" disabled={submitting}>
